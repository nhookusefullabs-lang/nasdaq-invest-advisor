...
//
// v9 파라미터 조정 changelog (PRD_Nasdaq9.md §4.4 워크플로 — README.md "백테스트 & 파라미터
// 조정 워크플로" 절 참고). 이 파일의 값을 바꿀 때는 반드시 아래 형식으로 한 줄씩 추가한다.
// 자동 파라미터 최적화로 인한 변경은 없다 — 모든 항목은 운영자가 backtest.json의
// Out-of-Sample 결과를 확인하고 수동으로 승인한 것이다.
//
// 형식: [YYYY-MM-DD] 변경 항목: 이전 값 → 이후 값 (근거: backtest.json/리포트 경로, 요약)
//
// (아직 실측 근거로 조정된 값 없음 — 이 워크플로가 처음 쓰일 때 아래에 항목을 추가할 것)
