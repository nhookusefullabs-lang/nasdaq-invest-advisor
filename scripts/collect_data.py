...
고정 기점(2021-01-01) 수집 — 2022년 실제 약세장을 평가 구간에 포함시켜 국면 체계
(regime.js)의 하락 국면 표본을 확보하기 위함 (PRD_Nasdaq8 §7 US-1(2년) →
PRD_Nasdaq9 §3 Should-2 US-9(3년) → PRD_Nasdaq11 §4.1 US-1(2021-01-01 고정 기점, 약 5.5년))

- yfinance로 나스닥100 종목의 2021-01-01 이후 일별 OHLCV를 1회 수집한다(약 5.5년치).
- 지표(RSI/MACD)의 워밍업(~35거래일), 52주 신고가/신저가(252거래일), SMA200 워밍업,
  백테스트 워밍업(252거래일) 제외 후 평가 구간을 충분히 확보하고, 시뮬레이션·화면
  표시는 웹앱에서 최근 63거래일만 사용한다.
...
# 수집 기점: "기간 길이"(period=Ny) 대신 고정 시작일을 쓴다(PRD_Nasdaq11 §4.1, US-1).
# 2021-01-01로 잡은 이유: v10의 국면 체계(regime.js)가 실측에서 하락 국면 표본을 거의
# 만나지 못했다(3년 데이터엔 진짜 약세장이 없었음) — 2022년 실제 약세장 전체를 평가
# 구간에 포함시켜야 국면 조건부 정책(하락 국면 완화 제외 등)을 제대로 검증할 수 있다.
# 10년 이상 소급은 하지 않는다: 이 프로젝트의 수집은 "현재 나스닥100/NGX 구성 종목"을
# 과거로 역산하는 방식이라, 더 오래된 과거로 갈수록 그 시점엔 아직 지수에 없었을
# 종목까지 포함하는 생존 편향이 누적된다 — 오래된 데이터를 더 얻는 한계효용이
# 생존 편향 비용 증가를 정당화하지 못해 보류한다(PRD_Nasdaq11 §2.4/§4.1).
# 한계 고지: 2022년 구간은 현재 구성 종목 기준으로 생존 편향이 상대적으로 크다
# (당시 지수에 없었을 종목이 "항상 있었던 것"처럼 역산되므로, 그 시기 성과가 실제보다
# 낙관적으로 보일 수 있다) — 국면 간 상대 비교 용도로만 해석할 것.
...
def collect_ndx() -> int:
    log(f"수집 시작: 나스닥100 후보 {len(NASDAQ100)}종목, 기점={START_DATE}, 간격={INTERVAL}")
    log("한계 고지: 2022년 구간은 현재 구성 종목 기준 역산이라 생존 편향이 상대적으로 큼 — 국면 간 상대 비교 용도로만 해석할 것")
